...
class Timelapse:
    ...
    async def newframe(self) -> None:
        ...
        try:
            cmdstatus = await scmd.run(timeout=4., verbose=False)
        except Exception:
    ...
    async def render(self, webrequest=None):
        ...
        if not filelist:
            ...
            try:
                ...
                timelapse_position = config_data.get("1").get("position", 0)
                brain_fps = config_data.get("1").get("fps", "MP4-15")
                brain_frequency = config_data.get("1").get("frequency", "1")
                if brain_fps == "MP4-15":
                    output_framerate = 15
                else:
                    output_framerate = 25
                # 20220121010735@False@1@15@.mp4
                camera_site = True if timelapse_position == 1 else False
                filename_extend = f"@{camera_site}@{brain_frequency}@{output_framerate}@"
            except Exception as e:
                logging.exception(e)
                output_framerate = 15
            ...
            # 文件数量低于默认的帧数，则复制最后一张，补充默认帧数两倍剩余的文件
            if len(filelist) < fps * 2:
                last_number = fps * 2
                logging.info("++++++++++++++++last_number:{} modify to last_number:{}".format(
                    self.framecount,
                    last_number
                ))
            else:
                logging.info("+++++++++++++++++no change, fps={}".format(fps))
            if last_number != self.framecount:
                for i in range(2, int(last_number) + 1):
                    source = os.path.join(self.temp_dir,
                                          "frame%06d.jpg" % (i - 1))
                    target = os.path.join(self.temp_dir, "frame%06d.jpg" % i)
                    if not os.path.exists(target):
                        logging.info("copyfile %s, %s" % (source, target))
            ...
            play_times = int(self.framecount / fps)
            outfile = outfile + f"{play_times}@"
